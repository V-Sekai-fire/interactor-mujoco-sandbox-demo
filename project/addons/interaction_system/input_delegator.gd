...
## The item the pointer is over. The manager takes it with every pointer event
## rather than tracking one itself, so that two hands can address different
## controls at once; whoever drives this delegator sets it.
var last_canvas_item: CanvasItem = null
...
func _input(event: InputEvent):
	...
	if me != null:
		if focused_interaction_manager != null:
			focused_interaction_manager.handle_pointer_moved_2d(
				last_canvas_item, last_canvas_item, me.global_position)
	var mb := event as InputEventMouseButton
	if mb != null:
		# handle_mouse_button reads the item to deliver to, so there is nothing
		# to deliver without one.
		if focused_interaction_manager != null and last_canvas_item != null:
			focused_interaction_manager.handle_mouse_button(last_canvas_item, mb)
